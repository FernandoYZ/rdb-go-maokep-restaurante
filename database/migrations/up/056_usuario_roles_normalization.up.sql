...
CREATE UNIQUE INDEX IF NOT EXISTS uq_usuario_roles_global
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_usuario_roles_sucursal
...
CREATE INDEX IF NOT EXISTS idx_usuario_roles_sucursal ON usuario_roles (id_sucursal)
    WHERE id_sucursal IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_usuario_roles_vigencia ON usuario_roles (efectivo_desde, efectivo_hasta);

-- Timestamp trigger
DROP TRIGGER IF EXISTS trg_actualizado_en_usuario_roles ON usuario_roles;
